...
public class RestoreDatabasePropertiesHandler extends AbstractTeamHandler {
	...
	private boolean replaceDatabaseProperties(String prefix, IProgressMonitor monitor) throws CoreException {

		String sourceFileName = String.format(SAVE_FILE_DBPROPS, prefix);

		IFile nsfFile = this.desProject.getProject().getFile(NSF_FILE_DBPROPS);
	...
	private boolean replaceIconNote(String prefix, IProgressMonitor monitor) throws CoreException {

		String sourceFileName = String.format(SAVE_FILE_ICONNOTE, prefix);

		IFile nsfFile = this.desProject.getProject().getFile(NSF_FILE_ICONNOTE);
		IFile diskFile = this.desProject.getProject().getFile(sourceFileName);
	...
	private boolean replaceDBIcon(String prefix, IProgressMonitor monitor) throws CoreException {

		String sourceFileName = String.format(SAVE_FILE_DBICON, prefix);

		IFile nsfFile = this.desProject.getProject().getFile(NSF_FILE_DBICON);
		IFile diskFile = this.desProject.getProject().getFile(sourceFileName);
	...
	private boolean replaceNsfContents(IFile diskFile, IFile nsfFile, IProgressMonitor monitor) throws CoreException {

		if (!diskFile.exists() || !nsfFile.exists()) {
			System.out.println("For this to work, both NSF and DISK File need to exist");
			return false;
		}
		...
		NotesDesignElement localNotesDesignElement = DominoResourcesPlugin.getNotesDesignElement(nsfFile);
		...
		try {
			...
			if (needsLock) {
				lockResult = NsfUtil.performLockOperation(localNotesDesignElement);
			}

			if ((!needsLock) || (lockResult)) {

				InputStream localInputStream = diskFile.getContents();
				StreamUtil.copyStream(localInputStream, localByteStreamCache.getOutputStream());
				localInputStream.close();

				// this.saveOptions is something to do with postProcessing /
				// maybe update timestamp?
				localNotesDesignElement.saveSyncContent(saveOptions, localByteStreamCache.toByteArray(),
			...
			SyncUtil.logErrorToConsole(
					StringUtil.format(ResourceHandler.getString("ImportSyncOperation.Errorimporting0.4"),
							new Object[] { diskFile }),
					localIOException);
		} catch (NsfException localNsfException) {
			SyncUtil.logErrorToConsole(
					StringUtil.format(ResourceHandler.getString("ImportSyncOperation.Errorimporting0.5"),
							new Object[] { diskFile }),
					localNsfException);
		}
		return false;
